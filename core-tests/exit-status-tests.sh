...
for fast in `seq 0 20`; do
  ...
  timeout 1 exit-status-test --result=False --slow=3 --fast=$fast --num-threads=4 --quiet
  [ $? -eq 1 ] || exit 1
  # When the number of slow tests >= number of threads, cannot find the failing
  # test
  timeout 1 exit-status-test --result=False --slow=4 --num-threads=4 --quiet
  [ $? -eq 124 ] || exit 1
  # Successful case (and no slow tests)
  timeout 1 exit-status-test --result=True --fast=$fast --num-threads=4 --quiet || exit 1
done
